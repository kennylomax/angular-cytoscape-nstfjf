...
const functions = (() => {
    ...
    if (typeof limit === 'undefined' || limit > 0) {
      ...
      if (typeof matches !== 'undefined') {
        while (
          ...
          (typeof limit === 'undefined' || count < limit)
        ) {
          result.push({
            match: matches.match,
            index: matches.start,
        ...
        while (index !== -1 && position < replacement.length) {
          ...
          if (dollarVal === '$') {
            ...
            if (regexMatch.groups.length === 0) {
              ...
              maxDigits =
                Math.floor(Math.log(regexMatch.groups.length) * Math.LOG10E) +
                1;
            }
            index = parseInt(
              replacement.substring(position, position + maxDigits),
              10
            );
            if (maxDigits > 1 && index > regexMatch.groups.length) {
              index = parseInt(
                replacement.substring(position, position + maxDigits - 1),
                10
              );
            }
            if (!isNaN(index)) {
  ...
  function formatNumber(value, picture, options) {
    ...
    var splitParts = function (subpicture) {
      var prefix = (function () {
        ...
        for (var ii = 0; ii < subpicture.length; ii++) {
          ...
          if (
            activeChars.indexOf(ch) !== -1 &&
            ch !== properties['exponent-separator']
          ) {
      ...
      var activePart = subpicture.substring(
        prefix.length,
        subpicture.length - suffix.length
      );
      var mantissaPart, exponentPart, integerPart, fractionalPart;
      var exponentPosition = subpicture.indexOf(
        properties['exponent-separator'],
        prefix.length
      );
      if (
        exponentPosition === -1 ||
        exponentPosition > subpicture.length - suffix.length
      ) {
      ...
      var decimalPosition = mantissaPart.indexOf(
        properties['decimal-separator']
      );
    ...
    var validate = function (parts) {
      ...
      if (
        decimalPos !== subpicture.lastIndexOf(properties['decimal-separator'])
      ) {
        error = 'D3081';
      }
      if (
        subpicture.indexOf(properties.percent) !==
        subpicture.lastIndexOf(properties.percent)
      ) {
        error = 'D3082';
      }
      if (
        subpicture.indexOf(properties['per-mille']) !==
        subpicture.lastIndexOf(properties['per-mille'])
      ) {
        error = 'D3083';
      }
      if (
      ...
      if (decimalPos !== -1) {
        if (
          subpicture.charAt(decimalPos - 1) ===
            properties['grouping-separator'] ||
        ...
        parts.integerPart.charAt(parts.integerPart.length - 1) ===
        properties['grouping-separator']
      ) {
        error = 'D3088';
      }
      if (
        subpicture.indexOf(
          properties['grouping-separator'] + properties['grouping-separator']
        ) !== -1
      ) {
        error = 'D3089';
      }
    ...
    var analyse = function (parts) {
      var getGroupingPositions = function (part, toLeft) {
        ...
        while (groupingPosition !== -1) {
          var charsToTheRight = (
            ...
            .filter(function (char) {
              return (
                decimalDigitFamily.indexOf(char) !== -1 ||
                char === properties.digit
              );
            }).length;
      ...
      var integerPartGroupingPositions = getGroupingPositions(
        parts.integerPart
      );
      ...
      var fractionalPartGroupingPositions = getGroupingPositions(
        parts.fractionalPart,
        true
      );

      var minimumIntegerPartSize = parts.integerPart
        .split('')
        .filter(function (char) {
          return decimalDigitFamily.indexOf(char) !== -1;
        }).length;
      ...
      var minimumFactionalPartSize = fractionalPartArray.filter(function (
        char
      ) {
        return decimalDigitFamily.indexOf(char) !== -1;
      }).length;
      var maximumFactionalPartSize = fractionalPartArray.filter(function (
        char
      ) {
        return (
          decimalDigitFamily.indexOf(char) !== -1 || char === properties.digit
        );
      }).length;
      ...
      if (exponentPresent) {
        minimumExponentSize = parts.exponentPart
          .split('')
          .filter(function (char) {
            return decimalDigitFamily.indexOf(char) !== -1;
          }).length;
    ...
    var padRight =
      pic.minimumFactionalPartSize - (stringValue.length - decimalPos - 1);
    stringValue =
      (padLeft > 0 ? new Array(padLeft + 1).join(zero_digit) : '') +
      stringValue;
    stringValue =
      stringValue +
      (padRight > 0 ? new Array(padRight + 1).join(zero_digit) : '');
    decimalPos = stringValue.indexOf(decimal_separator);
    // bullet 10:
    ...
    decimalPos = stringValue.indexOf(decimal_separator);
    if (
      pic.picture.indexOf(decimal_separator) === -1 ||
      decimalPos === stringValue.length - 1
    ) {
      stringValue = stringValue.substring(0, stringValue.length - 1);
    }
    // bullet 13:
    if (typeof exponent !== 'undefined') {
      ...
      if (padLeft > 0) {
        stringExponent =
          new Array(padLeft + 1).join(zero_digit) + stringExponent;
      }
      stringValue =
        stringValue +
        properties['exponent-separator'] +
        (exponent < 0 ? minus_sign : '') +
        stringExponent;
    }
    // bullet 14:
  ...
  function number(arg) {
    ...
    if (typeof arg === 'number') {
      ...
      typeof arg === 'string' &&
      /^(0[xX][0-9A-Fa-f]+)|(0[oO][0-7]+)|(0[bB][0-1]+)$/.test(arg)
    ) {
      result = Number(arg);
    } else if (arg === true) {
  ...
  function round(arg, precision) {
    ...
    if (precision) {
      ...
      result = +(
        value[0] +
        'e' +
        (value[1] ? +value[1] - precision : -precision)
      );
    }
