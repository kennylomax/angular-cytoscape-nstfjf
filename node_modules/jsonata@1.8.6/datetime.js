...
const dateTime = (function () {
  ...
  decades.forEach(function (word, index) {
    ...
    wordValues[lword.substring(0, word.length - 1) + 'ieth'] =
      wordValues[lword];
  });
  ...
  function lettersToDecimal(letters, aChar) {
    ...
    for (var i = 0; i < letters.length; i++) {
      decimal +=
        (letters.charCodeAt(letters.length - i - 1) - aCode + 1) *
        Math.pow(26, i);
    }
    return decimal;
  }
  ...
  function _formatInteger(value, format) {
    ...
    switch (format.primary) {
      case formats.LETTERS:
        formattedInteger = decimalToLetters(
          value,
          format.case === tcase.UPPER ? 'A' : 'a'
        );
        break;
      case formats.ROMAN:
      ...
      case formats.DECIMAL:
        ...
        if (format.zeroCode !== 0x30) {
          formattedInteger = stringToArray(formattedInteger)
            .map((code) => {
              return String.fromCodePoint(
                code.codePointAt(0) + format.zeroCode - 0x30
              );
            })
            .join('');
        }
        ...
        if (format.regular) {
          const n = Math.floor(
            (formattedInteger.length - 1) / format.groupingSeparators.position
          );
          for (let ii = n; ii > 0; ii--) {
            const pos =
              formattedInteger.length - ii * format.groupingSeparators.position;
            formattedInteger =
  ...
  const decimalGroups = [
    0x30, 0x0660, 0x06f0, 0x07c0, 0x0966, 0x09e6, 0x0a66, 0x0ae6, 0x0b66,
  ...
  function analyseIntegerPicture(picture) {
    ...
    switch (primaryFormat) {
      ...
      default: {
        ...
        const formatCodepoints = stringToArray(primaryFormat)
        ...
        if (def.presentation1[0] === 'n') {
          ...
          if ('YMD'.indexOf(def.component) !== -1) {
            ...
            if (def.width && def.width.max !== undefined) {
              ...
              var w =
                def.integerFormat.mandatoryDigits +
  ...
  const startOfFirstWeek = function (ym) {
    ...
    return dayOfJan1 > 4
      ? jan1 + (8 - dayOfJan1) * millisInADay
      : jan1 - (dayOfJan1 - 1) * millisInADay;
  };
    ...
    switch (component) {
      ...
      case 'w': {
        ...
        if (week > 4) {
          ...
          const startOfPreviousMonth = startOfFirstWeek(
    ...
    var formatComponent = function (date, markerSpec) {
      ...
      if ('YMDdFWwXxHhms'.indexOf(markerSpec.component) !== -1) {
        ...
        if (markerSpec.names) {
          ...
          if (
            markerSpec.width &&
            componentValue.length > markerSpec.width.max
          ) {
            componentValue = componentValue.substring(0, markerSpec.width.max);
          }
        } else {
          componentValue = _formatInteger(
            componentValue,
    ...
    if (formatSpec.type === 'datetime') {
        ...
        if (part.type === 'literal') {
          ...
          res.parse = function (value) {
            ...
            let offsetHours = 0,
              offsetMinutes = 0;
            if (separator) {
              offsetHours = Number.parseInt(
                value.substring(0, value.indexOf(separator.character))
              );
              offsetMinutes = Number.parseInt(
                value.substring(value.indexOf(separator.character) + 1)
              );
            } else {
      ...
      if (formatSpec.n && formatSpec.n > 0) {
        if (formatSpec.optionalDigits === 0) {
          ...
          occurrences = `{${formatSpec.n - formatSpec.optionalDigits},${
            formatSpec.n
          }}`;
        }
      } else {
      ...
      switch (formatSpec.primary) {
        ...
        case formats.WORDS:
          matcher.regex =
            '(?:' +
            Object.keys(wordValues).concat('and', '[\\-, ]').join('|') +
            ')+';
          matcher.parse = function (value) {
              ...
              digits = digits
                ...
                .map((char) =>
                  String.fromCodePoint(
                    char.codePointAt(0) - formatSpec.zeroCode + 0x30
                  )
                )
                .join('');
            }
  ...
  function parseDateTime(timestamp, picture) {
    ...
    const fullRegex =
      '^' +
      matchSpec.parts.map((part) => '(' + part.regex + ')').join('') +
      '$';
